...
Keywords: ${fields.keywords || "(not provided)"}`;
    ...
    tools: [
      {
        ...
        // Keep this focused: only the two tools we actually want Claude
        // reaching for, rather than the full AWS Knowledge tool surface
        // (list_regions, retrieve_skill, etc. aren't relevant here).
  ...
  // as separate text blocks — so join all of them rather than taking the
  // first one.
